...
/// reaches that boundary on its own: one transfer's offer plus every
/// in-flight chunk stays well inside one burst
/// (`crate::constants::MESH_QUOTA_BURST`), leaving the rest of it for
/// submits, replies and the other transfers open to the same peer. Nothing
/// bounds how many transfers a peer opens at once; enough of them in one
/// burst overrun the mailbox, the overrun chunks are dropped, and those
/// transfers stall until their hold expires.
...
// one window's worth of chunks, plus its offer, fits one sender's burst of
// the inbound mailbox — the DROP boundary the window exists to stay under.
const _: () = assert!(RELAY_BLOB_WINDOW_CHUNKS + 1 < crate::constants::MESH_QUOTA_BURST);
