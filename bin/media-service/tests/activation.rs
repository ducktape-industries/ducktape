...
fn start(listener: &TcpListener, credentials: &std::path::Path) -> Process {
    ...
    command
        .args([
            ...
            "media-activation",
            env!("CARGO_BIN_EXE_ducktape-media"),
        ])
    ...
    let process = Process(command.spawn().expect("start media executable"));
...
async fn refusal(address: std::net::SocketAddr, token: &str, caller: bool) -> u16 {
    let mut request = format!("ws://{address}/?channel=room")
        .into_client_request()
        .unwrap();
    request
        .headers_mut()
...
async fn held_listener_survives_process_replacement_and_authentication_fails_closed() {
    ...
    std::fs::write(
        ...
        r#"{"node_url":"http://127.0.0.1:0","account":7,"label":"media"}"#,
    )
    .unwrap();
    ...
    assert_eq!(
        refusal(address, &"a".repeat(64), true).await,
        503,
        "unavailable canonical roster refuses"
    );
    drop(first);
    assert!(
    ...
    assert_eq!(refusal(address, &"b".repeat(64), true).await, 401);
    assert_eq!(refusal(address, &"a".repeat(64), true).await, 503);
}
