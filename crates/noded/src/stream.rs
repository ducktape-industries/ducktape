...
/// the exact width of a run-output id: `crate::runs::dispatch_id_for` is a hex
...
async fn indexed_run_reader(
    ...
    match reply {
        crate::runs::view::RunsViewReply::Run(Some(detail)) => {
...
pub(crate) async fn pending_runs(
    ...
    handle
        .send(crate::NodeCommand::Query {
            ...
            req: crate::runs::encode_query(&crate::runs::RunsQuery::PendingRuns),
            reply,
        })
        .await
        .map_err(|_| "actor gone".to_string())?;
    let bytes = rx
        ...
        .map_err(|_| "reply dropped".to_string())?
        .map_err(|refused| refused.message)?;
    match crate::runs::decode_reply(&bytes)? {
        crate::runs::RunsReply::PendingRuns(runs) => Ok(runs),
