...
/// directory name — never a path. `crate::runs::validate_skills` now enforces this
...
/// [`crate::runs::is_skill_mount_name`], gates both sides so they cannot drift.
...
mod tests {
    ...
    fn mount_dir_name_agrees_with_the_consensus_side_predicate() {
        // `crate::runs::validate_skills` and this provisioner gate the SAME name
        // shape through ONE function (`crate::runs::is_skill_mount_name`) — this
