...
/// An object read costs no guest RUN. The object plane is a synchronous
/// substrate, so the import answers in place instead of pausing for a resolver:
/// a dispatch that reads a committed object runs the guest exactly as many times
/// as one that reads none. Counting `refs_bytes` counts runs — every round
/// re-seeds its committed view from the image exactly once — and under a
/// pause-and-replay object plane the reading dispatch would cost two.
#[tokio::test]
