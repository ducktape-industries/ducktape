...
//! staged overlay, an absent id resolves to `None` (the backing answers in
//! place, never a trap loop), the object-read budget rejects deterministically,
//! and staged puts are discarded on an aborted dispatch.
//!
...
async fn absent_object_resolves_to_none_not_a_trap_loop() {
    ...
    // an id that was never put: get/stat/has must answer absent out of the
    // backing, in place, never spin. a trap loop would blow the budget or hang
...
async fn object_read_budget_is_a_deterministic_rejection() {
    ...
    // carrying the object-specific budget message; the module is untouched. The
    // refusal lands ON the read that breaches the cap, inside the one run the
    // whole probe takes — object reads do not replay the guest.
