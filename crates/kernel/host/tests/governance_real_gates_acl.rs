...
use governance::{
    GovAction, GovMsg, GovQuery, GovReply, ProposalStatus, Standing as GovStanding,
    decode_reply as gov_decode, encode_msg as gov_encode, encode_query as gov_query,
};
        ...
        let err = submit_as(
            ...
            gov_encode(&GovMsg::Propose {
                ...
                action: GovAction::SetAclPolicy {
                    target: "governance".into(),
                    standing: Some(GovStanding::User),
                },
                voting_period: 100,
            }),
        )
    ...
    block_on(async {
        ...
        run_to_execute(
            ...
            GovAction::SetAclPolicy {
                target: "governance".into(),
                standing: Some(GovStanding::Node),
            },
        )
        .await
        ...
        assert_eq!(
            policy_for(&host, "governance").await,
            Some(AclStanding::Node)
        );
