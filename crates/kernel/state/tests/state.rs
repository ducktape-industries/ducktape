...
fn a_view_merges_storage_under_its_layers_top_layer_winning() {
    deterministic::Runner::default().start(|context| async move {
        ...
        let mut store = Store::open(context, dir.path(), [APP.to_owned()])
            .await
            .unwrap();
        store
            .commit(
...
fn a_committed_block_reaches_storage_and_commitment_and_survives_reopen() {
    deterministic::Runner::default().start(|context| async move {
        let dir = tempfile::tempdir().unwrap();
        let mut store = Store::open(context.child("first"), dir.path(), [APP.to_owned()])
            .await
            .unwrap();
...
fn a_crash_after_storage_and_before_commitment_is_reconciled_at_open() {
    deterministic::Runner::default().start(|context| async move {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mut torn = Store::open(context.child("torn"), dir.path(), [APP.to_owned()])
            .await
            .unwrap();
        ...
        let healed = Store::open(context.child("healed"), dir.path(), [APP.to_owned()])
            .await
            .unwrap();
